...
if (!query) {
  console.error(
    "Usage: npx tsx scripts/ingest-chronicling-america.ts --query='Jane Smith' [--limit=n]",
  );
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} historic-newspaper mention profile(s) from ${result.fetched} Chronicling America page(s).`,
  );
}
