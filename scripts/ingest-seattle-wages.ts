...
if (!lastName && !query) {
  console.error(
    "Usage: npm run ingest:seattle-wages -- --last-name=Smith [--first-name=Lori] [--limit=n]",
  );
  process.exit(1);
}

ingestSeattleWages({ firstName, lastName, query, limit })
  .then((result) => {
    console.log(`Fetched ${result.url}`);
    console.log(
      `Imported ${result.imported} Seattle wage profile(s) from ${result.fetched} row(s).`,
    );
  })
  .catch((error) => {
    console.error(error);
