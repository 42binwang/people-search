...
if (!query) {
  console.error(
    "Usage: npm run ingest:clinical-trials -- --query='Jane Smith' [--limit=n]",
  );
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} clinical trial personnel profile(s) from ${result.fetched} ClinicalTrials.gov study/studies.`,
  );
}
