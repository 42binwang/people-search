...
if (!query) {
  console.error("Usage: npm run ingest:openalex -- --query='Jane Smith' [--limit=n]");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} scholarly author profile(s) from ${result.fetched} OpenAlex result(s).`,
  );
}
