...
if (!query) {
  console.error(
    "Usage: npm run ingest:federal-register -- --query='Jane Smith' [--limit=n]",
  );
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} public-document mention profile(s) from ${result.fetched} Federal Register result(s).`,
  );
}
