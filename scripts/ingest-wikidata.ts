...
if (!query) {
  console.error("Usage: npm run ingest:wikidata -- --query='Jane Smith' [--limit=n]");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} public-knowledge profile(s) from ${result.fetched} Wikidata result(s).`,
  );
}
