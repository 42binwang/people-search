...
if (!query) {
  console.error(
    "Usage: npm run ingest:chicago-salaries -- --query='Jane Smith' [--limit=n]",
  );
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} Chicago employee profile(s) from ${result.fetched} payroll record(s).`,
  );
}
