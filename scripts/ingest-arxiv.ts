...
if (!query) {
  console.error("Usage: npm run ingest:arxiv -- --query='Jane Smith' [--limit=n]");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(
    `Imported ${result.imported} arXiv author profile(s) from ${result.fetched} arXiv entry/entries.`,
  );
}
